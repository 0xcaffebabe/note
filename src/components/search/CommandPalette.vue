  ...
  methods: {
    ...
    refreshCategory() {
      if (!this.kw) {
        ...
        this.items = CategoryService.getCategorySearchRecords()
          .map(c => this.categoryToItem(c, 'recent'))
        return
      }
      this.items = this.flatCategoryList
        .filter(c => CategoryService.categoryIsMatch(c, this.kw))
    ...
    async searchFulltext() {
      ...
      this.loading = true
      try {
        const result = await searchService.search(this.kw, 'algolia')
    ...
    highlight(raw: string): string {
      ...
      for (const part of kw.split(' ').filter(Boolean)) {
        try {
          result = result.replace(new RegExp(part.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), s => `<mark>${s}</mark>`)
