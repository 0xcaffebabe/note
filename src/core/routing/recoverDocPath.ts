...
// core 不依赖 vue-router: 仅描述本逻辑用到的最小路由形状(与 vue-router 的
// RouteLocationNormalized 结构兼容, 故 outer 层可直接把 to 传进来)。
...
// 自愈逻辑依赖的"文档是否存在"校验: 由 outer 注入(走 CategoryService 目录),
// 抽出为参数后本模块不再依赖重型服务, 仍可纯逻辑单测。
 ...
 */
export async function recoverDocPath(to: RouteLike, docIdExists: DocIdExists) {
  let path: string
  try {
