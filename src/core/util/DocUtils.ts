...
/**
 * 由文档 id 推断顶级分类: id 以 '-' 连接层级, 取首段(无则归 'other')。
 * 领域知识(docId 命名约定), 故归 core。
 */
function docCategory(nodeId: string): string {
  const parts = nodeId.split('-')
  return parts.length > 0 ? parts[0] : 'other'
}

/**
 * 路径(或文档url/名称) → 各层级段(剥结尾 .md, 大小写不敏感, 去空段)。
 * 面包屑/显示名的单一来源。
 */
function pathSegments(path: string): string[] {
  return (path || '').replace(/\.md$/i, '').split('/').filter(Boolean)
}

/** 路径 → 末段显示名(剥 .md, 取最后一个非空段)。 */
function pathLeafName(path: string): string {
  return pathSegments(path).pop() || ''
}

/** 文档 id → 各层级段(经 docId2Url 还原路径后取段)。 */
function docId2Segments(id: string): string[] {
  return pathSegments(docId2Url(id))
}

/** 文档 id → 末段显示名。 */
function docId2DisplayName(id: string): string {
  return pathLeafName(docId2Url(id))
}

export default {
  docId2Url, docUrl2Id, resloveDocUrl, docId2HtmlPath, htmlUrl2Id, routeDocId, docCategory,
  pathSegments, pathLeafName, docId2Segments, docId2DisplayName
}