...
export default defineComponent({
  ...
  methods: {
    ...
    clampToViewport() {
      ...
      el.style.left = Math.max(8, Math.min(el.offsetLeft, window.innerWidth - el.offsetWidth - 8)) + "px";
      el.style.top = Math.max(8, Math.min(el.offsetTop, window.innerHeight - el.offsetHeight - 8)) + "px";
    },
    async initDoc() {
      this.loading = true;
      try {
