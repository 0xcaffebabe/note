...
export default defineComponent({
  ...
  methods: {
    ...
    renderName(value: Category): string {
      const name = this.escapeHtml(value.name || '')
      ...
      for (const k of kw.split(" ").filter(v => v)) {
        const reg = new RegExp(this.escapeRegExp(this.escapeHtml(k)), 'gi')
      ...
      // 自身经拼音(或链接)命中 整名主色提示
      if (categoryService.categoryIsMatch(value, kw)) {
