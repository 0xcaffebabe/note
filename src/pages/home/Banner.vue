...
// 将日期转为相对时间标签 与 HomeQuickAccess 保持一致的口径
...
export default defineComponent({
  ...
  async created() {
    ...
    try {
      ...
      const categoryList = await CategoryService.getCompiledCategoryList()
      ...
      const updateLabel = last ? relativeDateLabel(last[1].date) : ''
    ...
    try {
      const list = await CategoryService.getOrderedDocList()
