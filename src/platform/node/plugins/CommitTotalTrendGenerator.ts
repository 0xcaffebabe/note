...
import fs from "fs";
import UrlConst from "../../../core/config/UrlConst";
import { shouldRegenerateTrend } from "../../../core/statistic/StatTransforms";
  ...
  return {
    ...
    async buildStart(options: any) {
      ...
      if (data.length == 0) {
        ...
        // 距离上一次生成时间大于7天的阈值判定已下沉 core/statistic(shouldRegenerateTrend)
        if (shouldRegenerateTrend(data, new Date().getTime())) {
