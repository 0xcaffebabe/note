...
import './style.less'
import NProgress from '@/adapters/browser/NProgress'
import createStore from '@/platform/web/store'
import { isMobile } from '@/platform/web/composables/useBreakpoint'
import { services } from '@/platform/web/app/container'
import { SERVICES_KEY } from '@/platform/web/app/useServices'
...
// 合成根: 把整套服务容器注入 Vue DI(provide) 与 Options API 全局属性($services)。
// $services 用 getter 读 live 模块绑定, 避免代码分割/模块求值顺序下取到尚未就绪的快照。
