...
import { categorySlug, countDocs, safeMatch } from '@/platform/web/util/CatalogUtils'
...
export default defineComponent({
  ...
  methods: {
    ...
    filterTree(list: Category[], kw: string): Category[] {
      return filterCategoryTree(list, cate => safeMatch(cate, kw))
    },
  ...
  async created() {
    ...
    try {
      this.noteCount = (await this.$services.api.getDescCommitDocList()).length
    } catch {
      // 统计行取不到则不渲染
    }
    this.loading = false
