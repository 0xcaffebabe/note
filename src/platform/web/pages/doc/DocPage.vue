...
import { RelatedLink } from "@/core/domain/RelatedLink";
...
import ImageViewer from "@/platform/web/components/ImageViewer.vue";
...
import { openExternal } from '@/adapters/browser/WindowAdapter';
import { isMobile, isTouch, isUltra } from '@/platform/web/composables/useBreakpoint';
...
export default defineComponent({
  ...
  computed: {
    ...
    docLinks(): RelatedLink[] {
      return this.$services.docService.resolveDocLinks(this.contentHtml, this.doc, this.relatedLinks.map(l => l.href));
    }
  },
  methods: {
    docUrl2Id(url: string) {
      return this.$services.docService.docUrl2Id(url);
    },
    ...
    async handleCopyDocContent(){
      try {
        ...
        const fileInfo = await this.$services.docService.getDocFileInfo(this.doc);
        if (fileInfo && fileInfo.content) {
          await writeText(fileInfo.content);
        ...
        try {
          ...
          if (markdownSection) {
            await writeText(markdownSection.textContent || '');
    ...
    openInEditor() {
      openExternal(`vscode://file/${config.localUrl}/${this.$services.docService.docId2Url(this.doc)}/`)
    },
    async showDoc(doc: string, headingId?: string, kw?: string) {
      // 将滚动条设置为上一次的状态(瞬时跳转 避免全局smooth造成长距离滚动动画)
      window.scrollTo({top: this.$services.docService.getDocReadRecord(doc), behavior: 'instant' as ScrollBehavior})
      this.loading = true;
      this.loadError = false;
      this.doc = doc;
      this.kw = kw || "";
      try {
        this.file = await this.$services.docService.getDocFileInfo(doc);
    ...
    postRender(headingId?: string, kw?: string, attempt: number = 0) {
      ...
      // 恢复阅读字号偏好
      const fontSize = this.$services.configService.get('fontSize')
      if (fontSize) {
      ...
      setTimeout(() => this.$services.docService.ensureQualityLoaded(), 3000)
    },
    ...
    generateTOC() {
      this.contentsList = this.$services.docService.getContent(this.contentHtml);
    },
    async randomizedReview() {
      // 如果没有关联文档，使用原来的随机策略
      const flatCategoryList = await this.$services.categoryService.getFlatCategoryList();
    ...
    async handleRandomReview() {
        ...
        try {
          // 获取知识网络数据
          const network = await this.$services.api.getKnowledgeNetwork();
          ...
          if (relatedDocs.length > 0) {
            ...
            try {
              await this.$services.docService.getDocFileInfo(selectedDoc)
            }catch (error) {
