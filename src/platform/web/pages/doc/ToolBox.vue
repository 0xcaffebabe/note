...
import { isMobile, isTouch, isFull } from "@/platform/web/composables/useBreakpoint";
...
export default defineComponent({
  ...
  data(){
    return {
      showDrawer: false,
      fontSize: (this.$services.configService.get('fontSize') as number) || 16,
  ...
  methods: {
    ...
    handleFontSizeChange(val: number) {
      ...
      this.$services.configService.set('fontSize', val);
    }
  },
  setup() {
    return {
  ...
  async created(){
    ...
    this.flatCategoryList = await this.$services.categoryService.getFlatCategoryList();
  },
  unmounted(){
