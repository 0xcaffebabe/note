...
export default defineComponent({
  ...
  methods: {
    ...
    async init() {
      ...
      const stream = (await this.$services.knowledgeNetworkService.getDocStream(this.doc)).flatMap(v => v)
      let knowledgeNetwork: KnowledgeNode[] = this.isPotential ? await this.$services.api.getPotentialKnowledgeNetwork() : await this.$services.api.getKnowledgeNetwork();
      const docService = this.$services.docService;
    ...
    assignClusterPositions(nodes: any[], categoryList: any[], chartDom: HTMLElement | null) {
      ...
      // 簇心几何为确定性逻辑, 抽至 core; DOM 尺寸由组件读出后传入
      ...
      if (Object.keys(clusterCenters).length === 0) return;
      ...
      nodes.forEach(node => {
        ...
        // 同类节点的小圆均布偏移为确定性逻辑(core), 随机抖动留在组件
        const { dx, dy } = nodeClusterOffset(idx, total);
        const jitter = 15;
        node.x = center.x + dx + (Math.random() - 0.5) * jitter;
        node.y = center.y + dy + (Math.random() - 0.5) * jitter;
      });
    },
