...
import { relativeLabelWithWeek } from "@/core/util/DateUtils";
...
export default defineComponent({
  ...
  methods: {
    ...
    formatRelative(dateStr: string): string {
      return relativeLabelWithWeek(dateStr);
    },
    ...
    quality(id: string): string{
      return this.$services.docService.calcQuanlityStr(id)
    },
    // 仅取分数部分做徽章 完整排名信息走title悬浮
    qualityScore(id: string): string {
      const q = this.$services.docService.getDocQuality(id);
    ...
    sortByQuantity() {
      if (this.displayMode == '倒序') {
        this.docList = this.docList.sort((b, a) => (this.$services.docService.getDocQuality(a[0])?.quality || 0) - (this.$services.docService.getDocQuality(b[0])?.quality || 0))
      }else {
        this.docList = this.docList.sort((a, b) => (this.$services.docService.getDocQuality(a[0])?.quality || 0) - (this.$services.docService.getDocQuality(b[0])?.quality || 0))
      }
    },
    sortByTime() {
      if (this.displayMode == '倒序') {
  ...
  async created() {
    this.docList = await this.$services.api.getDescCommitDocList()
    if (this.sort == '时间') {
        this.sortByTime();
      }else {
        this.sortByQuantity()
