...
import { localeDateCN } from "@/core/util/DateUtils";
import { computeKnowledgeGap, gapLevelClass, gapInterpretation } from "@/core/util/KnowledgeGapUtils";
import DocQuality from "@/core/domain/doc/DocQuality";
...
export default defineComponent({
  ...
  methods: {
    ...
    async generateScatterData(): Promise<{data: [number, number, string][], timeRange: {min: number, max: number, daysDiff: number}}> {
      const docUpdateList = await this.$services.api.getDescCommitDocList();
      const times = docUpdateList.map((v) => new Date(v[1].date).getTime());
      const minTime = Math.min(...times);
      const maxTime = Math.max(...times);
      const daysDiff = (maxTime - minTime) / (1000 * 3600 * 24); // 时间差（天数）

      const docQualityList = await this.$services.api.getDocQualityData();
      ...
      const option: EChartsOption = {
        ...
        tooltip: {
          formatter(params: any, ticket: string, callback: (ticket: string, html: string) => any): string {
            ...
            const result: Promise<DocFileInfo> | DocFileInfo = self.$services.docService.getDocFileInfo(id);
            if (result instanceof Promise) {
              result
                .then(file => {
                  callback(ticket, self.$services.docService.buildSummaryDocInfo(file) + sub);
                })
                .catch(err => {
                  callback(ticket, `${err}`);
                });
              ...
              return self.$services.docService.buildSummaryDocInfo(result) + sub;
            }
          }
        },
        series: [
    ...
    calculateKnowledgeGapIndex(timeRange: {min: number, max: number, daysDiff: number}, freshnessDays: number[]) {
      ...
      const { index, maxGap, gapStart, gapEnd } = computeKnowledgeGap(span, freshnessDays);
    ...
    formatDate(date: Date) {
      return localeDateCN(date);
    },

    // 根据断代指数获取等级样式类
    getGapIndexLevelClass(index: number) {
      return gapLevelClass(index);
    },

    // 获取断代指数的解释说明
    getGapIndexInterpretation(index: number) {
      return gapInterpretation(index);
    }
  },
});
