...
import templates from '@/platform/web/templates/llmTemplates';
import { LlmTemplate } from '@/platform/web/templates/llmTemplates';
...
export default defineComponent({
  ...
  methods: {
    async renderTemplate(template: LlmTemplate) {
      // 在创建时获取文档数据
      const file = await this.$services.docService.getDocFileInfo(this.doc);
      const contents = await this.$services.docService.getContentByDocId(this.doc);
      const categories = await this.$services.categoryService.getCategoryList();
      const tags = this.$services.tagService.getTagList();
      ...
      // 使用变量值生成模板内容
      return cleanTemplateLines(applyTemplateVariables(template.template, variableValues));
    },
    show() {
    ...
    contents2String(contents: any[]): string {
      return contentsToOutline(contents);
    },

    categories2String(categories: any[], level: number): string {
      return categoriesToOutline(categories, level);
    },
    ...
    updateTemplateContent() {
      if (this.currentTemplate) {
        // 使用变量值生成模板内容 + 清理空白
        this.query = cleanTemplateLines(applyTemplateVariables(this.currentTemplate.template, this.variableValues));
      }
    },
