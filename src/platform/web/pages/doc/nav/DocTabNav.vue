...
import { findByLink } from "@/core/util/CategoryUtils";
...
export default defineComponent({
  ...
  async created() {
    this.allCateList = await this.$services.categoryService.getCategoryList()
  },
  methods: {
    ...
    findRootCategoryFirstLetter(cateLink: string): string {
      const root = findByLink(this.allCateList, cateLink)
      if (!root) {
        return '📋'
      }
