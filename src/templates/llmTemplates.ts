...
function knowledgePrincipal(): string {
  return `
## 知识管理哲学
...
---
      ...
      `,
    generate: (variables: Record<string, string>) => {
      return variables.template
        .replace('{{docTitle}}', variables.docTitle)
        .replace('{{inputContent}}', variables.inputContent);
    }
  },
  {
    name: '文档升维处理',
...
- 所有标签必须与文本高度相关。
    ...
    generate: (variables: Record<string, string>) => {
      return variables.template
        .replace('{{docTitle}}', variables.docTitle)
        .replace('{{availableTags}}', variables.availableTags)
        .replace('{{inputContent}}', variables.inputContent);
    }
  },
  {
...
- 年份必须是行业公认趋势，不得凭空创造特定事件。
    ...
    generate: (variables: Record<string, string>) => {
      return variables.template
        .replace('{{docTitle}}', variables.docTitle)
        .replace('{{docOutline}}', variables.docOutline);
    }
  },
  {
    name: '历史背景',
...
- 所有趋势必须能从内容推导或来自通用行业规律。
  ...
  {
    name: '根据整个目录回答问题',
    value: 'answerByAllCategory',
    variables: [
      {
        name: 'knowledgeCatalog',
        label: '知识体系目录',
        value: '---\n{{allCategories}}\n---',
        description: '整个知识体系的目录'
      }
    ],
    template: `
你是一名知识体系分析专家。
...
【问题】（请在此处填写问题）
    ...
    generate: (variables: Record<string, string>) => {
      return variables.template
        .replace('{{knowledgeCatalog}}', variables.knowledgeCatalog);
    }
  },
  {
    name: '根据内容回答问题',
    value: 'ansuwerByContent',
    variables: [
      {
        name: 'docTitle',
        label: '文档标题',
