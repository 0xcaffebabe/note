...
// UA特征不会变化 可永久缓存; 视口宽度每次实时判定(matchMedia) 窗口缩放/旋转无需整页刷新
let uaMobileCheck: boolean | undefined;

export namespace SysUtils {
  export function isMobile(): boolean{
    if (typeof uaMobileCheck == 'undefined') {
      let check = false;
    ...
    return uaMobileCheck || (window.matchMedia ? window.matchMedia('(max-width: 820px)').matches : window.innerWidth <= 820);
  }
